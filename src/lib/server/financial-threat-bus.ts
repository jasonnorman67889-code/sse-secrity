...
function registerAdversaryMemory(
	...
	const bankBrand = String(
		monetizationLog.metadata.bankBrand ?? monetizationLog.metadata.brand ?? 'generic'
	);
...
function getAutonomousPlaybooks(
	entry: boolean,
	takeover: boolean,
	monetization: boolean
): string[] {
...
export async function evaluateFinancialThreatBus(input: FinancialBusInput) {
	...
	const entryLog = logs.find((log) =>
		['otp_forwarding_attempt', 'fake_portal_hit'].includes(log.type)
	);
	const takeoverLog = logs.find((log) => log.type === 'otp_harvest');
	const monetizationLog = logs.find((log) =>
		['card_verification_scam', 'bin_enumeration'].includes(log.type)
	);
	...
	const newDeviceSeen = logs.some((log) =>
		toBool(log.metadata.newDevice ?? log.metadata.firstSeenDevice)
	);
	...
	const mitigationTriggered =
		detectionTriggered || trustScore <= initialAdaptivePolicy.trustThreshold;
	const autonomousPlaybooks = mitigationTriggered
		? getAutonomousPlaybooks(entry, takeover, monetization)
		: [];
	const restoredTrust = mitigationTriggered
		? Math.max(8, Math.round((100 - trustScore) * 0.55))
		: 0;
	...
	const timeToContainmentMs = latestSpam?.timestamp
		...
		: null;
	const adaptivePolicy = evolveAdaptivePolicy(
		input.nodeId,
		timeToContainmentMs,
		mitigationTriggered
	);
	...
	const simulationArcState = inferSimulationArcState(
		delivery,
		detectionTriggered,
		mitigationTriggered
	);
