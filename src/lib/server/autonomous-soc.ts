...
function buildHypotheses(input: AutonomousSocInput, graphFinding: string, intelRisk: number) {
    ...
    if (intelRisk >= 70) {
        hypotheses.push('Indicator resembles known hostile infrastructure and should be prioritized for containment.');
    } else {
        hypotheses.push('Indicator currently ambiguous; monitor for recurrence across high-privilege users.');
    }

    if (input.geoName) {
        hypotheses.push(`Geo-velocity outlier anchored to ${input.geoName} requires cross-region policy validation.`);
    }

    return hypotheses;
}

export async function runAutonomousSocAnalysis(input: AutonomousSocInput): Promise<AutonomousSocAnalysis> {
    const mlAnomaly = normalizeAnomalyScore(input.anomalyScore);
    const graph = await investigateIdentityGraph(input.userId);

    const indicator = input.ipAddress ?? String(input.metadata?.ipAddress ?? input.metadata?.ip ?? 'unknown');
    const intel = inferThreatIntel(indicator, `${input.geoName ?? ''} ${String(input.metadata?.note ?? '')}`);

    const privileged = Boolean(input.metadata?.financeAdmin || input.metadata?.highPrivilege || false);
    const businessImpactRisk = clamp((privileged ? 72 : 45) + (graph.blastRadius > 4 ? 12 : 0), 0, 100);

    const decision = computeAutonomousSocDecision(mlAnomaly, graph.graphRisk, intel.risk, businessImpactRisk);
...
export async function runSocNaturalLanguageCommand(prompt: string): Promise<SocChatResult> {
    ...
    return {
        prompt,
        summary: 'Command not recognized as a remediation action. Use isolation or blast radius prompts.',
        suggestedAction:
