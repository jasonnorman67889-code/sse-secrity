...
function resolveTelegramProxy() {
	const proxyUrl =
		process.env.TELEGRAM_HTTPS_PROXY || process.env.HTTPS_PROXY || process.env.https_proxy || '';
...
async function sendTelegram(token, chatId, text) {
	...
	for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
		try {
			...
			console.log(
				`TELEGRAM: London Node Restored confirmation sent (attempt ${attempt}/${maxAttempts}).`
			);
			return;
		} catch (error) {
