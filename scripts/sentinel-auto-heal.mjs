...
async function runBootstrap(state, allowTaintedBootstrap) {
    ...
    if (!health.ok && !allowTaintedBootstrap) {
        ...
        console.error('SENTINEL: run with --allow-tainted-bootstrap to seed cache only from verifiable rows.');
        process.exitCode = 1;
        return;
    }
...
async function runSentinelCycle(state) {
    ...
    for (const row of health.tamperedRows) {
        ...
        healedRows.push({ ...row, workstation_id: golden.workstation_id, biometric_type: golden.biometric_type });
    }
...
main().catch((error) => {
    console.error('SENTINEL: fatal startup error:', error instanceof Error ? error.message : String(error));
    process.exitCode = 1;
});
