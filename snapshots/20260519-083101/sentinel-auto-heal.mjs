...
async function runSentinelCycle(state) {
	...
	if (healedRows.length === 0) {
		...
		console.log(
			`${stamp} SENTINEL: scan complete | entries=${health.totalEntries} | issues=${health.issues.length}`
		);
		return;
	}
	...
	for (const healed of healedRows) {
		...
		const nodeId = healed.workstation_id || 'UNKNOWN';
		console.log(
			`[Sentinel] Node ${nodeId} restored. Integrity: ${postHealHealth.ok ? '100%' : 'DEGRADED'}`
		);
	}
